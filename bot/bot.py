...
TARGET_CHANNELS = [
    x.strip()
    for x in os.getenv("TARGET_CHANNELS", "").split(",")
    if x.strip()
]
...
if not TARGET_CHANNELS:
    raise RuntimeError("TARGET_CHANNELS belum diisi")
...
async def main_loop():
    ...
    while True:
        ...
        for campaign in data.get("campaigns", []):
            ...
            interval = int(
                campaign.get("interval", 1800) or 1800
            )

            last_send = int(
                campaign.get("last_send", 0) or 0
            )
